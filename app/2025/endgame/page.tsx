		...
		<div className="flex flex-col justify-center items-center">
			...
			<div className="flex flex-col m-4 w-full items-center">
				<Checkbox
					className="m-4"
					value={data.park}
					onChange={(checked) => (data.park = checked)}
				>
					Park
				</Checkbox>
				<Checkbox
					className="m-4"
					value={data.shallow}
					onChange={(checked) => (data.shallow = checked)}
				>
					Shallow
				</Checkbox>
				<Checkbox
					className="m-4"
					value={data.deep}
					onChange={(checked) => (data.deep = checked)}
				>
					Deep
				</Checkbox>
				<TextField
					onChange={(e) => {
						data.timeClimbedAt = isNaN(e.target.valueAsNumber)
							? undefined
							: e.target.valueAsNumber;
						setClimbTime(data.timeClimbedAt);
						console.log(`${data.timeClimbedAt} ${climbTime}`);
					}}
					inputName="time-climbed-at"
					className="m-4"
					type="number"
					value={climbTime ? climbTime.toString() : ""}
					inputClassName="w-30 text-center"
				>
					Time climbed at
				</TextField>
			</div>
